...
type HttpOptions = {
  prepareHeaders?: (headers: Record<string, string>) => Promise<void> | void;
};

export async function http<T>(input: RequestInfo, init?: RequestInit, options?: HttpOptions): Promise<T> {
  const method = resolveMethod(init);
  const headers = resolveHeaders(init);
  applyCsrfHeader(headers, method);
  await options?.prepareHeaders?.(headers);
